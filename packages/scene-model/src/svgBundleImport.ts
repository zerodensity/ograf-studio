import { createAsset } from './factory';
...
export interface SvgBundleFile {
  name: string;
  type: string;
  size?: number;
  text: () => Promise<string>;
...
async function dataUri(file: SvgBundleFile, fallbackMime?: string): Promise<string> {
...
async function replaceRelativeReferences(
  source: string,
  filesByName: Map<string, SvgBundleFile>,
  warnings: Set<string>,
): Promise<string> {
...
export async function buildSvgBundle(files: SvgBundleFile[]): Promise<SvgBundleImportResult> {
